...
df_long <- df_long_com_esforco %>%
  mutate(abundancia_padronizada = individuos / esforco)
#

library(tidyverse)

df_epiphilini<- df_long %>%
  filter(tribo == "Epiphilini")

# Converta a coluna 'ano' para um fator
df_epiphilini$ano <- as.factor(df_epiphilini$ano)

# Calcule a mediana anual para cada ano
medianas_anuais <- tapply(df_epiphilini$abundancia_padronizada, df_epiphilini$ano, median)
...
for (ano_atual in names(medianas_anuais)) {
  ...
  wilcox_test_resultado <- wilcox.test(df_epiphilini$abundancia_padronizada[df_epiphilini$ano == ano_atual], mu = mediana_geral_anual, exact = FALSE)
...
erros_padrao <- sapply(names(medianas_anuais), function(ano) {
  dados_ano <- df_epiphilini$abundancia_padronizada[df_epiphilini$ano == ano]
...
p_epiphilini <- ggplot(df_plot, aes(x = Ano, y = MedianaAnual)) +
  geom_point(color = "gray55", size = 3) +
  geom_line(size = 1, group = 1, color = "gray55") +  # Use group = 1 para conectar todos os pontos
  geom_errorbar(aes(ymin = IntervaloConfiancaInf, ymax = IntervaloConfiancaSup), width = 0.2, color = "gray55") +
  ...
  ylim(0, 0.03) +  # Ajuste os limites conforme necessário
...
ggsave(filename = "mediana_epiphilini_ajustado_pontos_linha_2020.png",  # Escolha um nome adequado para o arquivo
       plot = p_epiphilini,  # O objeto ggplot
       device = "png",  # Pode ser "jpeg", "tiff", etc.
       path = "/Users/isabelafreitasoliveira/Documents/ICMBIO-COMOB/Dados_Monitora/Análise de dados/Monitora_Borboletas_Isabela/Destaque/Alto Tarauacá",  # Especifique o diretório onde você deseja salvar o gráfico
