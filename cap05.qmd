...
De 2014 a 2022 o protocolo básico de amostragem de mamíferos e aves (censo diurno em transecção linear) foi aplicado de forma integral ou parcial em 53 UCs federais^[Para efeito de análise, a RESEX do Rio Iriri e a ESEC da Terra do Meio foram tratadas como um bloco, de forma que os dados das duas UCs foram analisados de  forma integrada. Portanto, ao longo desse capítulo os resultados apresentados para a ESEC Terra do Meio na verdade expressam os dados das duas UCs.], totalizando 140 UAs (transecções lineares) em operação, distribuídas nos biomas Amazônia, Cerrado e Mata Atlântica.  


O esforço de amostragem nos nove anos considerados neste relatório correspondeu a 5.356 dias de campo (transecção/dia) e 25.602,55 km percorridos (@fig-esforco-acumulado-script), resultando em 22.985 registros de mamíferos de médio e grande porte e em 12.995 de aves terrestres cinegéticas.
...
No total, foram identificadas 131 espécies e 60 gêneros de mamíferos e 29 espécies e 13 gêneros de aves (Apêndices C e D), com 35 táxons ameaçados de extinção: 29 de mamíferos e 6 de aves, respectivamente. Registros de *Cavia* (preás),  *Crypturellus* (inhambus) e *Nothura* (codornas) foram validados sempre em nível de gênero. O esforço por UC e por bioma (Mata Atlântica, Cerrado e Amazônia) é apresentado na (@fig-esforco-total-uc-bioma-script).  
...
Dentre os mamíferos, os primatas destacaram-se como o grupo preponderante em número de registros e, exceto pelo gênero *Callibella* (sagui-anão), todos os demais gêneros de primatas com ocorrência no Brasil foram registrados. Setenta e duas espécies foram detectadas, sendo que 18 espécies são consideradas ameaçadas de extinção e duas apresentam deficiência de dados (DD) para avaliação.  
...
Verificou-se uma correlação positiva (r = 0,75; P < 0,001) entre as abundâncias (taxas médias de avistamento) de mamíferos e aves nas unidades de conservação amostradas (Figura X). Em termos gerais, esse é um resultado relativamente esperado, visto que o impacto das pressões negativas de origem antrópica deve, em linhas gerais, afetar de forma similar esses dois grupos de vertebrados.  
...
Duas espécies categorizadas como Deficientes de Dados (DDs), ou seja, sem informações suficientes para que seja feita uma avaliação adequada do seu estado de conservação, conforme o último ciclo de avaliação do estado de conservação das espécies da fauna brasileira (@SALVE_2024), foram registradas no presente esforço: os cervídeos *Mazama americana* e *M. nemorivaga*.   

O *Mazama americana* foi registrado no Parques Nacional do Iguaçu, no Paraná, e no Parque Nacional de Bodoquena, no Mato Grosso do Sul, unidades de conservação localizadas na Mata Atlântica e Cerrado, respectivamente. Além dessas, 35 outras unidades de conservação na Amazônia apresentaram registros dessa espécie DD. Já *Mazama nemorivaga* foi registrada apenas no bioma amazônico, como esperado, em 32 unidades de conservação. 
...
O Programa Monitora tem contribuído com registros importantes de *Cebus kaapori* (Caiarara), uma espécie de primata criticamente ameaçado de extinção. Essa espécie é endêmica do Brasil e em sua pequena área de distribuição temos apenas uma unidade de conservação de proteção integral, a REBIO do Gurupi.  Os registros dessa espécie na REBIO obtidos pelo Programa Monitora fornecem subsídios importantes para planejamento de iniciativas voltadas a sua conservação.  
...
Proporcionar registros de ocorrência e estimativas de abundância para algumas espécies DDs, a exemplo dos cervídeos apresentados como destaques neste relatório, será determinante para a recategorização desses táxons nos próximos ciclos de avaliação do risco de extinção de espécies brasileiras. Isso porque M. nemorivaga é considerada um complexo de espécies crípticas ainda por serem descritas e que necessita de pesquisas relacionadas a sua distribuição (@SALVE_2024). Alguns autores já sugerem que essa espécie deva ser um novo gênero monofilético em que Passalites nemorivagus é a espécie tipo (@MoralesDonoso_2023). Já M. americana, de forma semelhante, é considerada uma superespécie em que novas espécies poderão no futuro ser descritas (@Salviano_2017), o que dificulta a sua avaliação. 
