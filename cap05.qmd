...
#| fig.pos: 'H'
#| out.width: '70%'
#| fig-cap: "Táxons registrados acumulados (linha) e número de unidades de conservação (barras verticais) com protocolo básico de mamíferos e aves executado de 2014 a 2022."
...
Na @fig-especies-ameacadas-aves são apresentadas variações na abundância relativa de cinco espécies de aves amazônicas ameaçadas, *Aburria cujubi* (cujubi - VU) e quatro espécies de *Psophia*  (jacamins), para o conjunto de UCs do Programa Monitora. *Aburria cujubi* ocorre em 13 unidades de conservação, *Psophia viridis* (jacamim-de-costas-verdes - VU) em onze, *P. dextralis* (jacamim-de-costas-marrons - VU) em oito, *P. interjecta* (jacamim-do-xingu - EN) em duas e *P. obscura* (jacamim-de-costas-escuras - CR) em apenas uma. 
...
Duas espécies de cervídeos (*Mazama americana* e *M. nemorivaga*) categorizadas como Deficientes de Dados (DDs), ou seja, sem informações suficientes para que fosse feita uma avaliação adequada do seu estado de conservação no último ciclo avaliativo de espécies ameaçadas (@ICMBio_2024), foram registradas pelo Programa Monitora. 
...
Proporcionar registros de ocorrência e estimativas de abundância para algumas espécies com insuficiência de dados (DDs), a exemplo dos cervídeos apresentados como destaques neste relatório, será determinante para a recategorização desses táxons nos próximos ciclos de avaliação do risco de extinção de espécies brasileiras. Isso porque *M. nemorivaga* é considerada um complexo de espécies crípticas ainda por serem descritas e que necessita de pesquisas relacionadas a sua distribuição (@ICMBio_2024). Alguns autores já sugerem que essa espécie deva ser um novo gênero monofilético em que *Passalites nemorivagus* é a espécie tipo (@MoralesDonoso_2023). Já *M. americana*, de forma semelhante, é considerada uma superespécie em que novas espécies poderão no futuro ser descritas (@Salviano_2017), o que dificulta a sua avaliação.  
